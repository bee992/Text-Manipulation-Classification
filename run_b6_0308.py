...
def do_valid(net, valid_loader):
	...
	for t, batch in enumerate(tqdm(valid_loader)):
		...
		#---
		# print('\r %8d / %d  %s'%(valid_num, len(valid_loader),time_to_str(time.time() - start_timer,'sec')),end='',flush=True)
...
def run_train():
	...
	tampered_img_paths = "dataset/train/tampered/imgs"
	untampered_img_paths = "dataset/train/untampered/"
	...
	root_dir = '/hpc/data/home/bme/zhangzb1/Kaggle/Tianchi/classification/'
	out_dir  = root_dir + '/result/0308/B6-1024-v00a-RandomSampler-CosineLR-lr2e4'
	initial_checkpoint = None


	n_fold = 5
	kf = KFold(n_splits=n_fold, shuffle=True, random_state=1202)
	...
	for fold in range(n_fold):
		...
		all_dataset   = DTTDataset(df, augment=None)

		
		# pdb.set_trace()
		start_lr   = 2e-4 #0.0001
		batch_size = 12  #32 #32
		...
		# nextvit = NextViT(True, '/hpc/data/home/bme/zhangzb1/Kaggle/Tianchi/classification/nextvit_small_in1k_384.pth')
		...
		num_iteration = 40*len(train_loader)
		...
		log.write('                     |--------------VALID-----------|--------------ALL-----------|---- TRAIN/BATCH ----------------\n')
		log.write('rate     iter  epoch | loss  pF1  AUC  score  thres  | loss  pF1  AUC  score  thres  | time           \n')
		log.write('-----------------------------------------------------------------------------------------------------------\n')
		...
		def message(mode='print'):
			...
			text = \
				...
				'%4.4f %4.4f %4.4f %4.4f %4.4f | '%(*all_loss,) + \
				'%4.4f  %4.4f  %4.3f  | ' % (*loss,) + \
		...
		all_loss   = np.zeros(5,np.float32)
		...
		while iteration < num_iteration:
			for t, batch in enumerate((train_loader)):
				...
				if (iteration > 0 and iteration%iter_valid==0 and iteration > 5*len(train_loader)): 
					all_loss = do_valid(net, all_loader)
					pass	
				...
				if 0:
					...
					for b in range(batch_size):
						...
						# image_show_norm('image', m, min=0, max=1, )
						# image_show_norm('truth', onehot, min=0, max=1, )
						# image_show_norm('predict', p, min=0, max=1, )
						# cv2.waitKey(1)
